...
r_1s=[1.0]
r_2s=[1.0]
r_12s=[i for i in np.linspace(0, 20, 3)]  
tau_1s=[0.0]
tau_2s=[0.0]
D = 1000
epochs=100
...
Ss = [1 + np.random.normal(0, 1/np.sqrt(D))]


                        #is the file name structure below fine for you?
base_folder_name = f'T{Ts[0]}lr{lrs[0]}r_1{r_1s[0]}r_2{r_2s[0]}r_12{r_12s[0]}tau_1{tau_1s[0]}tau_2{tau_2s[0]}J_1{J_1_inits[0]:.3g}J_2{J_2_inits[0]:.3g}Q_1{Q_1_inits[0]:.3g}Q_2{Q_2_inits[0]:.3g}Q_12{Q_12_inits[0]:.3g}S{Ss[0]:.3g}epochs{epochs}savepoints{savepoints}'

"""
change actual_folder_name and experiment name to reflect the sweep being run
"""
experiment_name = 'test_ode'
actual_folder_name = f'T{Ts[0]}lr{lrs[0]}r_1{r_1s[0]}r_2{r_2s[0]}r_12{r_12s[0]}tau_1{tau_1s[0]}tau_2{tau_2s[0]}J_1{J_1_inits[0]:.3g}J_2{J_2_inits[0]:.3g}Q_1{Q_1_inits[0]:.3g}Q_2{Q_2_inits[0]:.3g}Q_12{Q_12_inits[0]:.3g}S{Ss[0]:.3g}epochs{epochs}savepoints{savepoints}'
